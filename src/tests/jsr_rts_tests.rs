...
fn build_program_parts(target_value: u8) -> Vec<Vec<u8>> {
    ...
    let set_x = test_helper::set_register_x_to_value(0);
    let compare_x_to_5 = test_helper::compare_x_to_value(target_value);
    let branch_for_loop = test_helper::branch_not_equal((5 as u8).wrapping_neg());

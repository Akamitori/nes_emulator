...
pub fn test_rom(pc_counter_start: u16, mirroring : Option<Mirroring>) -> Rom {
    let prg_rom_banks = 2;
    let chr_rom_banks = 1;
    ...
    let mirror=mirroring.unwrap_or(Mirroring::VERTICAL);

    let mut controlByte1=0x30;
